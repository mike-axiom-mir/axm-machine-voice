...
class NoticeSnapshotMachineChannelTests(unittest.TestCase):
    def setUp(self):
        self.example = ROOT / "examples" / "notice_snapshot.example.json"
    ...
    def test_notice_emits_through_same_snapshot_command(self):
        ...
        self.assertEqual(result["packet"]["kind"], "notice")
        self.assertEqual(result["packet"]["metadata"]["producer"], "grounded-notice/0.1")
        self.assertFalse(result["packet"]["claim"]["value"]["interpretation_claimed"])
    ...
    def test_untriggered_notice_is_machine_readable_silence(self):
        ...
        self.assertIn("explicit_notice_rule_not_triggered", result["reasons"])
        self.assertIsNone(result["packet"])

    def test_invalid_notice_input_stays_machine_readable_invalid(self):
        data = json.loads(self.example.read_text(encoding="utf-8"))
        data["signal"]["triggered"] = "yes"
        with TemporaryDirectory() as tmp:
