...
class NeedSnapshotMachineChannelTests(unittest.TestCase):
    def setUp(self):
        self.example = ROOT / "examples" / "need_snapshot.example.json"
    ...
    def test_need_snapshot_emits_through_same_snapshot_command(self):
        ...
        self.assertEqual(result["packet"]["kind"], "need")
        self.assertEqual(result["packet"]["event_id"], "need-snapshot-example-001")
        self.assertEqual(result["packet"]["metadata"]["producer"], "bounded-need/0.1")
        self.assertFalse(result["packet"]["claim"]["value"]["global_unavailability_claimed"])
    ...
    def test_all_required_inputs_present_is_machine_readable_silence(self):
        data = json.loads(self.example.read_text(encoding="utf-8"))
        data["available_inputs"].append({
        ...
        self.assertIn("bounded_inventory_contains_all_required_inputs", result["reasons"])
